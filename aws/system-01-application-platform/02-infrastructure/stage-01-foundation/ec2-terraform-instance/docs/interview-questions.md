...
Downloads provider plugins (AWS `~> 5.0`), initialises the backend, and creates the `.terraform/` directory. It must be run before any other command.

**What is the difference between `terraform plan` and `terraform apply`?**
`plan` generates an execution plan and shows what will change — it makes no changes. `apply` executes the plan and modifies real infrastructure.
